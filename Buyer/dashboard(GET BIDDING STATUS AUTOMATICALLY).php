        ...
        <div class="row d-flex justify-content-start">
          <?php 
            if(isset($_POST['search_button'])) {
              ...
              $fetch = mysqli_query($conn, "SELECT * FROM bidding JOIN product ON bidding.product_Id=product.product_Id JOIN category ON product.product_cat_Id=category.cat_id WHERE bidding.user_Id='$id' AND product.product_name LIKE '%$search_product%' ORDER BY product.bid_due_date ASC ");
                  ...
                  <div class="col-lg-3 col-md-3 col-sm-6 col-12">
                      <div class="card bg-light d-flex flex-fill">
                        ...
                        <div class="card-footer">
                            <p class="text-sm mb-0 float-left">Top Bid: <a href="#">₱<?php echo number_format($max_bid_price, 2, '.', ','); ?></a></p>
                            <p class="text-sm mb-0 float-right">You: <span id="bidStatus_<?php echo $productId; ?>"></span></p> <br>
                            ...
                            <p class="text-sm mb-0">Status: 
                              <?php if($myPrice >= $max_bid_price) : ?>
                                <a href="#" type="button">You are the highest bidder </a>
                        ...
                        if (distance <= 0) {
                            ...
                            var interval = setInterval(function() {
                              $.ajax({
                                  ...
                                  data: { getStatus: <?php echo $productId; ?>, user_Id:<?php echo $id; ?> },
                                  success: function(response) {
                                    // Parse the JSON response
                                    var biddingStatuses = JSON.parse(response);

                                    // Loop through the bidding statuses and update the UI
                                    for (var productId in biddingStatuses) {
                                      var biddingStatus = biddingStatuses[productId];

                                      if (biddingStatus == 0) {
                                        // The user has bidded
                                        document.getElementById("bidStatus_" + productId).innerHTML = "have bidded";
                                        document.getElementById("bidStatus_" + productId).className = "text-info";
                                      } else if (biddingStatus == 1) {
                                        // The user has won
                                        document.getElementById("bidStatus_" + productId).innerHTML = "Won";
                                        document.getElementById("bidStatus_" + productId).className = "text-success";
                                      } else if (biddingStatus == 2) {
                                        // The user has lost
                                        document.getElementById("bidStatus_" + productId).innerHTML = "Lost";
                                        document.getElementById("bidStatus_" + productId).className = "text-danger";
                                      }
                                    }
                                  },
          ...
          <?php } //END OF WHILE LOOP ?>  
                ...
                <div class="col-12 text-center">
                  <hr>
                  <p>You have reached the end of the searched list</p>
                </div> 
