      ...
      <div class="container">
        <div class="row d-flex justify-content-start">
          ...
          <?php } } /*END OF IF ISSET FUNCTION*/
            ...
            $fetch = mysqli_query($conn, "SELECT * FROM product JOIN category ON product.product_cat_Id=category.cat_id WHERE product.product_status=0 AND product.bid_due_date > '$date_today' ORDER BY category.cat_id, RAND()");
              if(mysqli_num_rows($fetch) > 0) {
                while ($row = mysqli_fetch_array($fetch)) {
                  $productId = $row['product_Id'];
                  $countdownId = "countdown-" . $productId;
