              ...
              <div class="card-body p-3">
                 <table id="example1" class="table table-bordered table-hover text-sm">
                  ...
                  <tbody id="users_data">
                      <?php 
                        $i = 1;
                        $sql = mysqli_query($conn, "SELECT * FROM product JOIN category ON product.product_cat_Id=category.cat_id JOIN users ON product.user_Id=users.user_Id");
                        while ($row = mysqli_fetch_array($sql)) {

                          $pro_Id = $row['product_Id'];

                          // GET WINNER INFO
                          $win = mysqli_query($conn, "SELECT * FROM bidding_winner JOIN users ON bidding_winner.user_Id=users.user_Id WHERE product_Id='$pro_Id'");
                          // $winnerId = mysqli_fetch_array($);
                      ?>
                      <tr>
                        <td class="text-center">
                            <a data-toggle="modal" data-target="#viewphoto<?php echo $row['product_Id']; ?>">
                              <img src="../images-users/<?php echo $row['image']; ?>" alt="" width="40" height="40" class="img-circle d-block m-auto">
                            </a href="">
                        </td>
